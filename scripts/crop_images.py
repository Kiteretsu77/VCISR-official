...
class worker:
    ...
    def process(self, path, opt, usm_sharper):
        ...
        if random_num == -1:
            ...
            shift_offset_h = random.randint(0, height - crop_size * (height//crop_size))
            shift_offset_w = random.randint(0, width - crop_size * (width//crop_size))
        else:
        ...
        for (h, w) in res_store:
            ...
            cv2.imwrite(osp.join(opt['save_folder'], f'img_{self.output_index:05d}.png'), cropped_img, [cv2.IMWRITE_PNG_COMPRESSION, 0])     # Save in lossless mode
            ...
            if opt['usm_save_folder'] != None:
                ...
                cv2.imwrite(osp.join(opt['usm_save_folder'], f'img_{self.output_index:05d}.png'), cropped_sharpened_img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_folder', nargs='+', type=str, default='datasets/Flickr2K_HR', help='Input folder') # TODO: support multiple image input
